...
var BattleDao = function(){
    ...
    this.insert = function(values, callback){
        db.query("INSERT INTO user (id_joueur1, id_joueur2, score1, score1, date, duree) VALUES (\"" + values[0] + "\", \"" + values[1] + "\", \"" + values[2] +"\""+"\", \"" + values[3] +"\""+"\", \"" + values[4] +"\""+"\", \"" + values[5] +"\")",function(err,rows){
            if(err){
                callback(err, null);
            }else{
                callback(null, rows);
	...
	this.findByKey = function(key, callback){
        db.all("SELECT * FROM battle WHERE id_battle =?",key, callback)
    };
